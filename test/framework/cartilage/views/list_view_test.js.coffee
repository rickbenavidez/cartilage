...
test "should render collection correctly", ->
  ...
  ok $('.list-view > ul.list-view-items-container'), "ListView container was not created."
  equal $('.list-view > ul.list-view-items-container > li').length, 3, "@testListView should have 3 items."
  equal @testListView.selected.length, 0, "testListView should have 0 selected items."

test "should trigger select event for single-select ListView", ->
  expect(1)
  @testListView.prepare()
  $('#testElement').html @testListView.render().el 
...
test "should trigger deselect event for single-select ListView", ->
  expect(1)
  @testListView.prepare()
  $('#testElement').html @testListView.render().el 
